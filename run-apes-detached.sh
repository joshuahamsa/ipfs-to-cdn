...
# Run the apes upload script in a detached screen session
# Usage: ./run-apes-detached.sh [additional arguments]
# Example: ./run-apes-detached.sh --resume-from 2500 --max-retries 10 --skip-cdn-check

echo "Starting apes upload in detached screen session..."
echo "To reconnect to the session later, run: screen -r apes-upload"
...
DEFAULT_ARGS="--resume-from 2406 --max-retries 5 --retry-delay 10 --download-timeout 300 --log-file ipfs-to-cdn-apes-detached.log"
...
echo "  screen -r apes-upload    # Reconnect to the session"
echo "  screen -ls               # List all screen sessions"
echo "  screen -S apes-upload -X quit  # Kill the session"
echo ""
echo "Example usage:"
echo "  ./run-apes-detached.sh                                    # Use defaults"
echo "  ./run-apes-detached.sh --resume-from 2500                 # Resume from 2500"
echo "  ./run-apes-detached.sh --skip-cdn-check --max-retries 10  # Skip CDN check, more retries"