#!/usr/bin/env python3
"""
Convert CSV NFT data to JSON metadata files and upload directly to Bunny CDN.
...
def extract_attributes_from_row(row):
    """Extract attributes from CSV row data."""
    attributes = []

    # Map CSV columns to trait types
    attribute_mapping = {
        ...
        "Attribute Clothes": "Clothes",
        "Attribute Earring": "Earring"
    }
...
def create_metadata_json(row):
    """Create JSON metadata from CSV row data."""
    # Extract edition number from Name field
    # (e.g., "Bored Apes XRP Club #9992" -> "9992")
    name = row.get("Name", "")
    edition = ""
    if "#" in name:
        edition = name.split("#")[-1].strip()
    ...
    metadata = {
        ...
        "description": row.get(
            "Description",
            "The Bored Apes XRP Club is a collection of 10,000 unique "
            "Bored Ape NFTs living on the XRP ledger."
        ),
        "image": row.get("Image", ""),
        "edition": edition,
        "date": 1667650922062,  # Default date
        ...
        "external_link": row.get("External_link", "https://x-apes.com/"),
        "category": row.get("Category", "Collectables"),
        "collection": {
...
def process_nft_row(session, row, storage_zone, access_key, region_host,
    ...
    try:
        ...
        dest_key = f"{dest_prefix}{edition}.json"

        # Upload to CDN
        bunny_put_json(session, storage_zone, access_key, region_host,
                       dest_key, metadata)

        return True, edition, None
...
def main():
    ap = argparse.ArgumentParser(
        description="Convert CSV NFT data to JSON metadata and upload to "
                    "Bunny CDN."
    )
    ap.add_argument("--csv-file", default=DEFAULT_CSV_FILE,
                    help="Path to CSV file containing NFT data")
